...
SMODS.Consumable:take_ownership('wheel_of_fortune',
	{
		...
		use = function(self, card, area, copier)
			if (pseudorandom('wheel_of_fortune') < G.GAME.probabilities.normal / card.ability.extra.odds and not next(SMODS.find_card("j_hmlt_hope"))) or (next(SMODS.find_card("j_hmlt_hope")) and pseudorandom('wheel_of_fortune') < (G.GAME.probabilities.normal * 2) / card.ability.extra.odds) then
				local editionless_jokers = SMODS.Edition:get_edition_cards(G.jokers, true)
